...
class MatrixChainMultiplication {
	...
	// multiplication
	static int matrixMultiplication_Memorization(int[] arr, int i, int j, int[][] dp) {
		// Base condition
		if (i == j) {
			return 0;
		}
		...
		for (int k = i; k <= j - 1; k++) {
			int operations = matrixMultiplication_Memorization(arr, i, k, dp)
					+ matrixMultiplication_Memorization(arr, k + 1, j, dp) + arr[i - 1] * arr[k] * arr[j];
	...
	static int matrixMultiplication_Tabulation(int[] arr, int N) {
		int[][] dp = new int[N][N];
