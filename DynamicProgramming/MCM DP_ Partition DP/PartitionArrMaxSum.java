...
public class PartitionArrMaxSum {
	...
	static int maxSumAfterPartitioning_Memorization(int[] num, int k) {
		int n = num.length;
		int[] dp = new int[n];
		Arrays.fill(dp, -1);
		return f(0, num, k, dp);
	...
	static int maxSumAfterPartitioning_Tabulation(int[] num, int k) {
		int n = num.length;
		int[] dp = new int[n + 1];
	...
	public static void main(String[] args) {
		...
		int k = 3;
		int maxSum = maxSumAfterPartitioning_Memorization(num, k);
